///draw_rectangle_cd_barfill_icon_sprite_ext(x1, y1, x2, y2, value,count,sprite,color,symcolor,alpha)
var v, x1, y1, x2, y2, xm, ym,sprite,count,color,symcolor,alpha;
...
draw_sprite_ext(s_v_icon_background,0,xm,ym,1,1,0,color,alpha) //60x60 canvas for these
draw_sprite_ext(sprite,0,xm,ym,1,1,0,symcolor,alpha);

//Draw icon fill only in above region w/ alpha 1 and up to the bar fill
draw_sprite_part_ext(s_v_icon_background,0,0,0,59,(1-v)*59,xm-30,ym-30,1,1,COLORS[7],alpha*.45)
...
if count > 1{
    draw_sprite_ext(s_v_icon_background,0,xm+30-4,y1+4,24/60,24/60,0,
        COLORS[0],alpha) //24x24 canvas for these
    draw_text_colour(xm+30-4,y1,string(count),
        COLORS[6],COLORS[6],
        COLORS[6],COLORS[6],alpha)
}
