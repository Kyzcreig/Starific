...
///scr_spawner_chance_init(specific_chance)

/* If generating all these arrays iteratively like this is a problem, e.g. it lags on your Samsung S3
    we can try declaring big arrays before we set the weight and outcome arrays e.g.
    weight[30] = 0;
    out[30] = 0;
    and see if that makes a difference, or perhaps it would be cleaner to have a templat array
    that's like 40's and copy it, since you can do that like TWEENGMS
    and that avoids the possibility that arrays aren't cleared to 0.
...
// Declare Vars
var weight, out;
var specific_chance = argument0;



////////////////////////////////////////////////
if specific_chance == "" or string_pos(specific_chance,"deflector_types") {
    ...
    if VC_ENABLED{
        ...
        //weight[w] = .0050;//.01; //.02
            ...
            */
        weight[w] = .0075;//.01; //.02
        if scr_gamemod_get_index("extra_coins", 8) > 0 {
            weight[w] *= scr_gamemod_get_index("extra_coins", 10);
        }
        switch MODE {
            case MODES.MOVES:
    ...
    if scr_gamemod_get_index("extra_bombs", 8) > 0 {
        weight[w] += .4 * (scr_gamemod_get_index("extra_bombs", 10) - 1); //add 10% of bomb density
    } 
    if scr_gamemod_get_index("extra_powers", 8) > 0 {
        weight[w] += .6 * (scr_gamemod_get_index("extra_powers", 10) - 1); //add 10% of bomb density
    }
...
if specific_chance == "" or string_pos(specific_chance,"cash") {
...
if specific_chance == "" or string_pos(specific_chance,"basic") {
...
if specific_chance == "" or string_pos(specific_chance,"special_types") { 
    ...
    if scr_gamemod_get_index("extra_bombs", 8) > 0 {
        weight[w-1] *= scr_gamemod_get_index("extra_bombs", 10);
    } 
    if scr_gamemod_get_index("extra_powers", 8) > 0 {
        weight[w-1] /= scr_gamemod_get_index("extra_powers", 10); 
    }
    weight[w] = .15;   out[w++] = 1;  //powerups (instant effects)
       
    if scr_gamemod_get_index("upper_exile", 8) <= 0 or 
       scr_gamemod_get_index("downer_exile", 8) <= 0 
    {
...
if specific_chance == "" or string_pos(specific_chance,"bombs") {
...
if specific_chance == "" or string_pos(specific_chance,"instant") {
    ...
    weight[w] = .0025;                    out[w++] = obj_powerup_bomb_detonate;  //EVALUATE
    ...
    weight[w] = .00;                    out[w++] = obj_powerup_bomb_detonate; 
    ...
    weight[w] = .005;                    out[w++] = obj_powerup_powerupsclear; 
    weight[w] = .005;                    out[w++] = obj_powerup_powerdownsclear; 
    weight[w] = .005;                    out[w++] = obj_powerup_bomb_detonate; 
    //weight[w] = .015;                 out[w++] = obj_powerup_1down;
    //weight[w] = .015;                out[w++] = obj_powerup_1up; 
...
if specific_chance == "" or string_pos(specific_chance,"duration") {
    ...
    if scr_gamemod_get_index("upper_exile", 8) <= 0 {
        ...
        weight[w] = .0025;    out[w++] = obj_powerup_multidensity;
    ...
    if scr_gamemod_get_index("upper_exile", 8) <= 0 {
        ...
        weight[w] = .0075;    out[w++] = obj_powerup_multidensity;
    ...
    if scr_gamemod_get_index("upper_exile", 8) <= 0 {
        ...
        weight[w] = .0150;    out[w++] = obj_powerup_multidensity;
...
///scr_spawner_outcome(chance_key)
