///GMTwitter_tweet_ext(text, URL, hashtags)
/*
...
for (i=1;i<=49;i++) //Replaces special characters
   {
   s1=string_replace_all(s1,c_rep[i,0],c_rep[i,1]);
   if url!="" url=string_replace_all(url,c_rep[i,0],c_rep[i,1]);
   if ht!="" ht=string_replace_all(ht,c_rep[i,0],c_rep[i,1]);
   }

link="http://twitter.com/intent/tweet?text="+s1;
if url!="" link+="&url="+url;
if ht!="" link+="&hashtags="+ht;
