///scr_board_mixer_promote(foundList, chance)
...
for (i=0;i<ds_list_size(foundList);i++){
    ...
    if currentCell >= DENSITY and object_is_ancestor(currentCell.object_index,obj_reflector_parent_basic){
        ...
        //Else Add to Empties
            ...
            scr_field_empty_add(gridXY[0],gridXY[1]); 
        }*/
    }
    //Case Empty or Respawning or Special Deflector
    else{
        //Particle effect fake unspawn
        part_particles_create_color(PSYS_FIELD_LAYER, fieldXY[0], fieldXY[1], p_unspawn,power_type_colors(1,0),1); //not sure about color
    ...
    if !cellSpawning{
            ...
            fieldXY[0],fieldXY[1],p_spawn,power_type_colors(1,0),1); //not sure what color to use here
    }
}
